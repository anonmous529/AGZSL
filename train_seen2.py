...
from models.resnet101 import resnet101_spp
import utils 
...
class train(object):
    def __init__(self,device,cfg_file,summaryFolder):
        ...
        attSplit = loadmat('/dataset/ZSL/xlsa17/data/'+self.cfg['dataset']['name']+'/att_splits.mat') 
        res101 = loadmat('/dataset/ZSL/xlsa17/data/'+self.cfg['dataset']['name']+'/res101.mat')
        ...
        #self.cls_weight = self.cal_clsweight(seen_labels)
    ...
    #def cal_clsweight(self,labels):
    #    cls_num = {}
    #    max_num = 0
    #    cls_weight = []
    #    for l in np.unique(labels):
    #        cls_loc = labels == l
    #        cls_num[l] = len(labels[cls_loc])
    #        if max_num < cls_num[l]:
    #            max_num = cls_num[l]
    #    for l in sorted(cls_num):
    #        cls_weight.append(np.sqrt(max_num/cls_num[l]))
    #    return cls_weight
    ...
    def loadDataFromCustom(self,dataType):
        ...
        with open('/home/tim/project/ZSL/dataset/'+self.cfg['dataset']['name']+'/extractedFeatures/dvbeExtracted.pkl','rb') as f:
    ...
    def loss(self,):
        ...
        """
        #self.lossf = nn.CrossEntropyLoss(weight=torch.FloatTensor(self.cls_weight)).to(self.device)
            ...
            for images,targets in tqdm(dataloader):
                ...
                for c in torch.unique(targets):
                    ...
                    try:
                        ...
                        #spp_loss_cls[c.cpu().detach().tolist()].extend(spp_loss[loc].cpu().detach().tolist())
                    except(KeyError):
                        ...
                        #spp_loss_cls[c.cpu().detach().tolist()] = spp_loss[loc].cpu().detach().tolist()
    ...
    def trainModel(self,chkFile,recordFile,resume=False):
        ...
        bestHs = 0
        if resume:
            bestAcc=self.loadChk(chkFile)
        else:
            # record training info
            with open(recordFile,'w') as f:

                f.write('%5s,%17s,%17s,%17s,%17s,%17s\n'%('epoch','loss','seenAcc','seenSpploss','unseenAcc','unseenSppLoss'))
        ...
        for epoch in range(self.cfg['model_hyp']['start_epoch'],self.cfg['model_hyp']['epochs']):
            ...
            if testUnseenEval['generalAccs'] > bestAcc:
                bestAcc = testUnseenEval['generalAccs']
                self.saveModel(epoch,Info,self.model,self.optimizer,self.scheduler,trainSeenEval,testSeenEval,testUnseenEval,H,chkFile+'_best_unseen.pt')
                best_unseenInfo = Info

            if H > bestH:
                bestH = H
                self.saveModel(epoch,Info,self.model,self.optimizer,self.scheduler,trainSeenEval,testSeenEval,testUnseenEval,H,chkFile+'_best_H.pt')
                best_HInfo = Info

            if Hs > bestHs:
                bestHs = Hs
                self.saveModel(epoch,Info,self.model,self.optimizer,self.scheduler,trainSeenEval,testSeenEval,testUnseenEval,H,chkFile+'_best_Hs.pt')
                best_HsInfo = Info
            #
